...
export function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <article className="relative group overflow-hidden rounded-lg border border-hairline bg-card transition-all duration-300 hover:border-brand-green/35 hover:shadow-md hover:shadow-brand-green/5 hover:-translate-y-1 h-full flex flex-col">
      {/* Cover Image */}
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-1">
        ...
        <h3 className="text-lg sm:text-xl font-bold text-ink group-hover:text-brand-green-deep transition-colors line-clamp-2 mb-2">
          <Link href={`/blog/${post.slug}`} className="after:absolute after:inset-0 after:z-10">
            {post.title}
          </Link>
        </h3>
